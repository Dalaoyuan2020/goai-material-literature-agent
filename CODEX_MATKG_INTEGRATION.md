...
已用真实文件头和 MatKG 官方生成笔记本核验：`SUBRELOBJ.csv` 的列是 `Subject,Object,Rel,Count`。其中 `Rel` 是两个 NER 实体类型的组合（例如 `CHM-SMT`），**不是**“掺杂/取代”等语义关系；`Count` 是同一文本片段中两个实体共同出现的不同 DOI 数量。聚合后的 CSV 没有保留这些 DOI 的具体值，因此不得把 `Count` 当成逐条可溯源证据，也不得把 `Rel` 硬映射成 R1-R9。转换器须保留原标签，并将这类记录标为 MatKG 聚合共现弱关系。
