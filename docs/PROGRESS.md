...
- 保留 Track A 分支较新的算法、Materials Project 交叉验证、测试和审计产物；
- UI 分支只引入 `workbench/`、`src/ui_bridge.py`、工作流入口和演示文档，没有用旧运行快照覆盖比赛证据；
- 后续 DSH 阅读器适配在同一私有比赛仓库的 `codex/dsh-reading-integration` 分支推进，不另建产品远端仓库。
