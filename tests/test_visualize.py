...
class VisualizationTests(unittest.TestCase):
    def test_machine_report_totals_keep_evidence_tiers_separate(self):
        ...
        self.assertEqual(sum(l2["core_relation_counts"].values()), 81)
        self.assertEqual(sum(l2["matkg_relation_counts"].values()), 210)
        self.assertEqual(len(pipeline["L3_rules"]["non_degenerate_evidence_records"]), 326)
    ...
    def test_generate_all_expected_files(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertIn("53 DOI-backed papers", mermaid)
            self.assertIn("210 aggregate edges", mermaid)
            self.assertIn("80 retained hypotheses", mermaid)
